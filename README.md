...
Today, the project already includes usable footer/deck-sync tooling and a first equation workflow. The broader goal is to grow it into a full academic presentation toolkit with equations, captions, references, and reusable academic blocks.
...
- Save a footer as a reusable template in Figma Slides
- Auto-generate page numbers from a selected text node
- Apply or sync a footer template across slides
- Exclude slides and control numbering start
- Bind footer text as content sources and map them to source slides by page range
...
- section titles and footer metadata need to stay in sync
